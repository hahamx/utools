...
var (
	...
	DefaultCornTable   = "corn"
	DefaultVideoTable  = "videos"
	DefaultScriptTable = "python"
	DefaultLiveTable   = "live" //直播表
	AllowCronTables    = []string{DefaultCornTable, DefaultVideoTable, DefaultScriptTable}
	DefaultPriKey      = "id"

	//按顺序存入和解析cron 结构体
	OrderFields = []string{"name", "spec", "command"}
	//直播字段
	LiveFields = []string{"name", "url", "start_at"}
)
